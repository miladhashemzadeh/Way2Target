package com.vampyreworld.w2t.domain.usecase.decision.impl
...
class GetDecisionsUseCaseImpl(
    private val repository: DecisionRepository
) : GetDecisionsUseCase {
    override fun invoke(goalId: Long?, challengeId: Long?): Flow<List<Decision>> {
        return repository.getDecisions(goalId, challengeId)
    }
}

class SaveDecisionUseCaseImpl(
    private val repository: DecisionRepository
) : SaveDecisionUseCase {
    override suspend fun invoke(decision: Decision) {
